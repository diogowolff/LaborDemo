...
Random.seed!(1234)
df = df[shuffle(1:nrow(df))[1:5000], :]
...
initPars = [102.007, -24.051, 6.20181, -0.00433641]
initPars = [82.924, 6.61095, -5.62199, -0.0881276]

### Local optimization
a = optimize(loglike, initPars, Optim.Options(show_trace = true))
a = optimize(loglike, initPars, BFGS(), Optim.Options(show_trace = true, iterations = 50))
